...
Run the examples with `shiny::runApp(system.file("shiny-examples/<EXAMPLE>", package = "dtdatasources"))` 
...
### Contributing
If you would like to add a data source, please contribute! Also, feedback, 
bug reports, fixes, and feature requests are welcome, see the [current issues](http://github.com/grahamrp/dtdatasources/issues). 
...
to be in a *dataframe* on the server. Sometimes we don't want to put the entire
dataset into a dataframe, for example if it is very big, or if it naturally
belongs in a database or behind an API.
...
If you want to use these examples to create your own implementations of DT data
sources, take a look at `rest_api_election.R` for a REST API example. This function adapts the
...
For SQL implementations, take a look at `sqlite.R` and `sql_filter_factory.R` for
an example.
